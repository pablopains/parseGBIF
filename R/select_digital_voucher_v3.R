...
#' @name select_digital_voucher_v3
#'
...
#' }
#' @export
select_digital_voucher_v3 <-  function(occ = NA,
                                    occ_gbif_issue = NA,
                                    ...
                                    silence = TRUE)
{
  ...
  # for
  {
    ...
    occ$Ctrl_coordinates_validated_by_gbif_issue
...

    {
      ...
      r <- occ$Ctrl_key_family_recordedBy_recordNumber

      fam <-str_sub(r,1, str_locate(r, '_')[1]-1) %>% ifelse(is.na(.), "",.)

      ind_incomplete_key <- (str_sub(r, str_count(r), str_count(r)) == '_' |
                               grepl('__', r) |
                               grepl('UNKNOWN-COLLECTOR',r))

      FAMILY__ <-  FAMILY__recordNumber <- FAMILY_recordedBy_ <- rep(FALSE,NROW(r))


      r[ind_incomplete_key==TRUE]

      #
      FAMILY__[ind_incomplete_key==TRUE] <- grepl('__', r[ind_incomplete_key==TRUE]) & str_locate(r[ind_incomplete_key==TRUE], '__')[2] == str_count(r[ind_incomplete_key==TRUE])
      FAMILY__[ind_incomplete_key==TRUE] <- ifelse(is.na(FAMILY__[ind_incomplete_key==TRUE]), FALSE, FAMILY__[ind_incomplete_key==TRUE])
      if(any(FAMILY__[ind_incomplete_key==TRUE]==TRUE)==FALSE)
      {
        FAMILY__ <- rep(FALSE,NROW(r))
      }
      r[FAMILY__[ind_incomplete_key==TRUE]==TRUE]

      #
      FAMILY_recordedBy_ <- str_sub(r, str_count(r), str_count(r)) == '_' & ! grepl('__', r)
      r[FAMILY_recordedBy_]

      #
      FAMILY__recordNumber <- (grepl('__', r) | grepl('UNKNOWN-COLLECTOR',r))
      r[FAMILY__recordNumber]
      any(ind_incomplete_key == (FAMILY__|FAMILY_recordedBy_|FAMILY__recordNumber))==FALSE

      #
      sp_name <- rep('',NROW(occ[ind_incomplete_key==TRUE, ]))
      sp_name <- ifelse(occ$wcvp_taxon_status[ind_incomplete_key==TRUE] == 'Accepted',
                        occ$wcvp_taxon_name[ind_incomplete_key==TRUE] %>% as.character(),
                        '')

      occ[ind_incomplete_key==TRUE, ] <- occ[ind_incomplete_key==TRUE, ] %>%
        dplyr::mutate(parseGBIF_digital_voucher = TRUE,
                      ...
                      parseGBIF_duplicates_grouping_status = ifelse(FAMILY__[ind_incomplete_key==TRUE]==TRUE,
                                                                    'not groupable: no recordedBy and no recordNumber',
                                                                    ifelse(FAMILY_recordedBy_[ind_incomplete_key==TRUE]==TRUE,
                                                                           'not groupable: no recordNumber ',
                                                                           ifelse(FAMILY__recordNumber[ind_incomplete_key==TRUE]==TRUE,
    ...
    occ_incomplete_key <- occ[ind_incomplete_key==TRUE,]

    occ <- occ[ind_incomplete_key==FALSE,]

    # recordedBy_unique <- occ$Ctrl_key_family_recordedBy_recordNumber[ind_incomplete_key==FALSE] %>% unique()
    ...
    for (r in recordedBy_unique)
    {
      ...
      #     grepl('__', r) |
      #     grepl('UNKNOWN-COLLECTOR',r))==TRUE)
      # {
      #   print(paste0(r, ' não aprupável'))
      #   next
      # }
        ...
        # nomes coordenadas
        {
          ...
          # nomes para agrupaveis
          {
            ...
            }else
            {
              ...
              taxon_name_sample <- table(occ_key$wcvp_taxon_name,
