  ...
  # app
  {
...

    {
...

      {
        ...
        # col_all <<- c('parseGBIF_dataset_result',
        #            'Ctrl_key_family_recordedBy_recordNumber',
        #
        #            'parseGBIF_useful_for_spatial_analysis',
        #            'parseGBIF_sample_taxon_name_status',
        ...
        dashboardPage(
          ...
          dashboardSidebar(width = 350,
                           ...
                           box(status = "primary", width = 12,
                               ...
                               actionButton("selectBtn", "Select a species", icon = icon("play")),
                           )
          ),
                       ...
                       # tab all
                       {
                         tabPanel(icon("table"),
                                  box(title = "3. Table",
                                      ...
                                      box(status = "primary", width = 12,
                                          title = 'Table of Unique Collection Events',# background = 'navy', # red, yellow, aqua, blue, light-blue, green, navy, teal, olive, lime, orange, fuchsia, purple, maroon, black.

                                          uiOutput("picker2"),

                                          # actionButton("save_tableBtn", "Save table", icon = icon("download")),

                                          downloadButton("save_tableDownload", "Download table"),
          ...
          get_url <- eventReactive(input$url_gbif_Btn,
                                   {
                                     withProgress(message = 'Processing...', style = 'notification', value = 0.5, {
                                       ...
                                       # gbif_doi_url <- 'https://www.gbif.org/occurrence/download/0067051-240626123714530'
                                       gbif_doi_url <- input$url_gbif_Input
                                       folder <- path_results <- input$folder_Input #'C:\\R_temp' #
                                       # parseGBIF::download_gbif_data_from_doi(gbif_doi_url, folder = path_results)
...

                                       {
                                         ...
                                         zip.file <- utils::unzip(path_file_zip_GBIF, exdir = folder)
                                       ...
                                       # downloaded_occurrence_file <<- paste0(folder,'\\occurrence.txt')

                                       downloaded_occurrence_file <<- zip.file[1]
                                       ...
                                       occ <<- readr::read_delim(file = downloaded_occurrence_file,
                                                                 delim = '\t',
                ...
                withProgress(message = 'Processing...', style = 'notification', value = 0.1,
                             {
                               ...
                               occ_tmp <- readr::read_delim(file = downloaded_occurrence_file,
                                                            delim = '\t',
                                                            locale = readr::locale(encoding = "UTF-8"),
                                                            show_col_types = FALSE)
              ...
              withProgress(message = 'Processing...', style = 'notification', value = 0.1,
                           {
                             ...
                             # url <- paste0("<a href='",sprintf(base_url,occ$Ctrl_gbifID),"'>",occ$Ctrl_gbifID,"</a>")
      ...
      # table
      {
        ...
        output$picker2 <- renderUI({
          ...
          pickerInput(inputId = 'pick2',
                      label = 'Choose fields to view and export',
                      choices = c('gbif_url',col_all),
                      options = list(`actions-box` = TRUE),
                      multiple = T,
                      selected = col_sel,
        ...
        output$occ_all_parseGBIFContents <- DT::renderDataTable(
...

          {
            ...
            occ_tab <<- parsegbifLoad2() %>%
              dplyr::select(input$pick2)
        ...
        output$save_tableDownload <- downloadHandler(
          ...
          content = function(file) {
            ...
            #             dplyr::select(input$pick2),
            #           file, row.names = FALSE, fileEncoding = "UTF-8", na = "")
            write.csv(parsegbifLoad2()[input[["occ_all_parseGBIFContents_rows_all"]], ] %>%
                        dplyr::select(input$pick2),
                      file, row.names = FALSE, fileEncoding = "UTF-8", na = "")
          })
        ...
        #   info = input$occ_all_parseGBIFContents_url_gbif_clicked
        #
