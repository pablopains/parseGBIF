#' @title Selecting the master digital voucher with multiple backbones
#' @name select_digital_voucher_backbones
#'
#' @description
...
#'
#' @param occ_gbif_issue
#' Data frame. Result of function `extract_gbif_issue()$occ_gbif_issue`.
#'
#' @param occ_wcvp_check_name
#' Data frame. Result of function `batch_checkName_wcvp()$occ_wcvp_check_name`.
#'
#' @param occ_flora_funga_brasil_name
#' Data frame. Result of function `batch_checkName_flora_funga_brasil()$occ_flora_funga_brasil_name`.
#' Required when using 'FFB' backbone.
#'
#' @param occ_collectorsDictionary
#' Data frame. Result of function `update_collectorsDictionary()$occ_collectorsDictionary`.
#'
#' @param enumOccurrenceIssue
#' Data frame. An enumeration of validation rules for single occurrence records by GBIF.
#' If NA, will use built-in data(EnumOccurrenceIssue).
#'
#' @param silence
#' Logical. If `TRUE`, does not display progress messages. Default is `FALSE`.
#'
#' @param backbone
#' Character. Taxonomic backbone to use:
#' - `'WCVP'`: World Checklist of Vascular Plants (default)
#' - `'FFB'`: Flora Funga Brasil
#'
#' @details
...
#' - `occ_digital_voucher`: Processed data with all fields
#' - `occ_results`: Data with only result fields
#'
#' @author
...
#'
#' @seealso
#' [`batch_checkName_wcvp()`] for name checking against WCVP,
#' [`batch_checkName_flora_funga_brasil()`] for name checking against Flora Funga Brasil,
...
#' }
#'
#' @importFrom dplyr mutate filter arrange select distinct_all left_join
#' @importFrom stringr str_sub str_count str_locate
#' @importFrom utils data
#' @export
select_digital_voucher_backbones <- function(occ_tmp= NA,
                                              occ_gbif_issue = NA,
...
{
  {
    ...
    if(toupper(backbone)=='FFB')
    {
      occ<- cbind(occ_gbif_issue, occ_in, occ_flora_funga_brasil_name, occ_collectorsDictionary)


      # # occ_tmp$wcvp_taxon_rank %>% unique()
      # occ$fb2020_taxonRank <- ifelse(is.na(occ$fb2020_taxonRank),'',occ$fb2020_taxonRank)
      #
      # # occ_tmp$wcvp_taxon_status %>% unique()
      # occ$fb2020_taxonomicStatus <- ifelse(is.na(occ$fb2020_taxonomicStatus),'',occ$fb2020_taxonomicStatus)
      #



      # occ$wcvp_plant_name_id <- occ$fb2020_taxonID
      # occ$wcvp_taxon_name <- occ$fb2020_scientificNamewithoutAuthorship
      # occ$wcvp_taxon_status <- occ$fb2020_taxonomicStatus
      # occ$wcvp_searchNotes <- occ$fb2020_searchNotes
      # occ$wcvp_taxon_rank <- occ$fb2020_taxonRank
      # occ$wcvp_family <- occ$fb2020_family
      # occ$wcvp_taxon_authors <- occ$fb2020_scientificNameAuthorship
      # occ$wcvp_reviewed <- rep('',NROW(occ))
      # occ$wcvp_searchedName <- occ$fb2020_searchedName


      occ$backbone_plant_name_id <- occ$fb2020_taxonID
      occ$backbone_taxon_name <- occ$fb2020_scientificNamewithoutAuthorship
      occ$backbone_taxon_status <- occ$fb2020_taxonomicStatus
      occ$backbone_searchNotes <- occ$fb2020_searchNotes
      occ$backbone_taxon_rank <- occ$fb2020_taxonRank
      occ$backbone_family <- occ$fb2020_family
      occ$backbone_taxon_authors <- occ$fb2020_scientificNameAuthorship
      occ$backbone_reviewed <- rep('',NROW(occ))
      occ$backbone_searchedName <- occ$fb2020_searchedName



    }

    if(toupper(backbone)=='WCVP')
    {
      occ<- cbind(occ_gbif_issue, occ_in, occ_wcvp_check_name, occ_collectorsDictionary)

      occ$backbone_plant_name_id <- occ$wcvp_plant_name_id
      occ$backbone_taxon_name <- occ$wcvp_taxon_name
      occ$backbone_taxon_status <- occ$wcvp_taxon_status
      occ$backbone_searchNotes <- occ$wcvp_searchNotes
      occ$backbone_taxon_rank <- occ$wcvp_taxon_rank
      occ$backbone_family <- occ$wcvp_family
      occ$backbone_taxon_authors <- occ$wcvp_taxon_authors
      occ$backbone_reviewed <- occ$wcvp_reviewed
      occ$backbone_searchedName <- occ$wcvp_searchedName


      # occ_tmp$wcvp_taxon_rank %>% unique()
      occ$backbone_taxon_rank <- ifelse(is.na(occ$backbone_taxon_rank),'',occ$backbone_taxon_rank)

      # occ_tmp$backbone_taxon_status %>% unique()
      occ$backbone_taxon_status <- ifelse(is.na(occ$backbone_taxon_status),'',occ$backbone_taxon_status)
    ...
    for (r in recordedBy_unique)
    {
      # seleção voucher digital
      {
        ...
        if(FAMILY__ == TRUE | FAMILY__recordNumber == TRUE | FAMILY_recordedBy_== TRUE )
        {
          ...
          sp_name <- ifelse(occ_key$backbone_taxon_status %in% c('Accepted','NOME_ACEITO'),
                            occ_key$backbone_taxon_name %>% as.character(),
                            '')


          sp_id <- ifelse(occ_key$backbone_taxon_status %in% c('Accepted','NOME_ACEITO'),
                          occ_key$backbone_plant_name_id %>% as.character(),
                          '')
          ...
          occ[index_occ==TRUE, ] <- occ_key %>%
            dplyr::mutate(parseGBIF_digital_voucher = TRUE,
                          ...
                          # new
                          parseGBIF_backbone_plant_name_id = sp_id,

                          # aqui
                          parseGBIF_sample_taxon_name = sp_name,
                          # parseGBIF_unidentified_sample = ifelse(sp_name == '', TRUE,FALSE),
                          parseGBIF_unidentified_sample = ifelse(is.na(sp_name) | trimws(sp_name) == '', TRUE, FALSE),
        ...
        }else
        {
          # digital voucher
          {
            ...
            occ[index_occ==TRUE, ] <- occ_key %>% #dplyr::select(-backbone_taxon_name_and_backbone_plant_name_id) %>%
              dplyr::mutate(parseGBIF_duplicates_grouping_status = 'groupable',
                            parseGBIF_duplicates = num_records > 1,
                            parseGBIF_num_duplicates = num_records,
          ...
          occ_key$backbone_taxon_name_and_backbone_plant_name_id <- paste0(occ_key$backbone_taxon_name,
                                                                   ';',
                                                                   occ_key$backbone_plant_name_id)
          ...
          # nomes coordenadas
          {
            ...
            # nomes para agrupaveis
            {
              ...
              # if(!any(is.na(occ[index_occ==TRUE, ]$backbone_taxon_name) == FALSE)) # any ???
              if(!any(is.na(occ_key$backbone_taxon_name) == FALSE)) # any ???
              {
              ...
              }else
              {
                ...
                taxon_name_sample <- table(occ_key$backbone_taxon_name_and_backbone_plant_name_id, #occ_key$backbone_taxon_name,
                                           occ_key$backbone_taxon_status,
                                           exclude = NA) %>%
                  data.frame() %>%
                  dplyr::filter(Freq > 0) %>%
                ...
                if(num_taxon_name==0)
                {
                  ...
                  print(occ[index_occ==TRUE, ]$backbone_taxon_name)
                ...
                # aqui aqui
                if(num_taxon_name==1 & taxon_name_sample$Var2[1] %in% c('Accepted','NOME_ACEITO'))#,'Updated'))
                {
                  ...
                  occ[index_occ==TRUE, ] <- occ_key %>% dplyr::select(-backbone_taxon_name_and_backbone_plant_name_id) %>%
                    dplyr::mutate(
                      ...
                      # new
                      parseGBIF_backbone_plant_name_id = sp_name_id[,2],
                ...
                if(num_taxon_name>1)
                {
                  ...
                  for(ii in 1:NROW(taxon_name_sample))
                  {
                    ...
                    if(taxon_name_sample$Var2[ii] %in% c('Accepted', 'NOME_ACEITO'))#,'Updated'))
                    {
  ...
  # in dup out_to_recover
  {
    ...
    occ_in_2 <- occ %>% #cbind(occ_in, ifelse(toupper(backbone)=='WCVP',occ_backbone_check_name,occ_flora_funga_brasil_name), occ_collectorsDictionary, occ2) %>%
    ...
    occ_dup <- occ %>% #cbind(occ_in, ifelse(toupper(backbone)=='WCVP',occ_backbone_check_name,occ_flora_funga_brasil_name), occ_collectorsDictionary, occ2) %>%
      dplyr::filter(parseGBIF_digital_voucher == FALSE)

    occ_out_to_recover <- occ %>% #cbind(occ_in, ifelse(toupper(backbone)=='WCVP',occ_backbone_check_name,occ_flora_funga_brasil_name), occ_collectorsDictionary, occ2) %>%
      dplyr::filter(parseGBIF_digital_voucher == TRUE,
                    (parseGBIF_unidentified_sample == TRUE |
                       parseGBIF_useful_for_spatial_analysis == FALSE))
    ...
    ####
    {
      ...
      # occ$parseGBIF_dataset_result <- ""
      #
      # index <- occ$parseGBIF_digital_voucher == TRUE &
      #   occ$parseGBIF_unidentified_sample == FALSE &
      #   occ$parseGBIF_useful_for_spatial_analysis == TRUE
      #
      # occ$parseGBIF_dataset_result[index] <-'useable'
      #
      # index <- occ$parseGBIF_digital_voucher == FALSE
      #
      # occ$parseGBIF_dataset_result[index] <-'duplicate'
      #
      # index <- occ$parseGBIF_digital_voucher == TRUE &
      #                 (occ$parseGBIF_unidentified_sample == TRUE |
      #                    occ$parseGBIF_useful_for_spatial_analysis == FALSE)
      #
      # occ$parseGBIF_dataset_result[index] <-'unusable'
      #
      # occ_all <- occ
    }
    ####
    ...
    # # wcvp sample
    {
      ...
      print(NROW(occ))
      xn <- occ %>%
        dplyr::mutate(backbone_plant_name_id = as.character(backbone_plant_name_id)) %>%
        dplyr::select(backbone_plant_name_id,
                      backbone_taxon_rank,
      ...
      occ_all <- left_join(occ_all,
                           xn,
                           by = 'parseGBIF_backbone_plant_name_id') %>%
