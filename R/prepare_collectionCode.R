...
prepare_collectionCode <- function(occ = NA, herbarium_data = NA, quiet = FALSE)
{
  ...
  pares_instituicao_colecao <- pares_instituicao_colecao %>%
    mutate(
      is_acronym = case_when(
        ...
        code_in_catalogNumber %in% herbarium_acronyms & code_in_catalogNumber !="" ~ "code_in_catalogNumber",
      ...
      Ctrl_collectionCode_standardized = case_when(
        ...
        is_acronym == "code_in_catalogNumber" ~ code_in_catalogNumber,
  ...
  occ$parseGBIF_collectionCode_standardized <- ""
  ...
  occ_code_in_catalogNumber <- occ$code_in_catalogNumber

  occ <- occ %>% select(-code_in_catalogNumber)
  ...
  for(i in seq_len(tot)) {
    ...
    } else if(is_acronym[i] == "code_in_catalogNumber"){
      index <- which(occ_code_in_catalogNumber == current_code)
      if(length(index) > 0) {
        occ$parseGBIF_collectionCode_standardized[index] <- current_code
      }
    }
  ...
  standardized_count <- sum(!occ$parseGBIF_collectionCode_standardized=="")
