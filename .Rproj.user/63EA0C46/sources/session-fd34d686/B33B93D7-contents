                                    ...
                                    {box(title = "5. Summary of results",
                                         ...
                                         fluidRow(
                                           column(width = 12,
                                                  tabsetPanel(
                                                    tabPanel("Summary", 
                                                             DT::dataTableOutput( 'SummarySummaryContents'),
                                                             br(),
                                                             downloadButton("summ_summary_Download", "Download summary")),
                                                    tabPanel("Results",
                                                             DT::dataTableOutput("summary_Contents"),
                                                             br(),
                                                             downloadButton("occ_summary_Download", "Download results"))
                                                  )))
        ...
        geospatialVerification <- eventReactive(input$geospatialVerificationBtn,
                        ...
                        ORDER BY COUNT(ID_Count) DESC") 
                                                    ...
                                                    centroidOccurenceCount_result <- centroidOccurenceCount_result %>%
                                                      dplyr::select(all_of('centroidOccurenceCount',
                                                                           'point_cent'))
        ...
        extractIssue <- eventReactive(input$extractIssueBtn,
                                      {
                                        withProgress(message = 'Processing...', style = 'notification', value = 0.5, {
                                          ...
                                          occ_gbif_issue <- parseGBIF::extract_gbif_issue(occ = data())
        ...
        applyWCVP <- eventReactive(input$applyWCVPBtn,
                                   {
                                     withProgress(message = 'Processing...1', style = 'notification', value = 0.5, {
                                       ...
                                       wcvp_names <-  rWCVPdata::wcvp_names %>%
                                         data.frame(stringsAsFactors = FALSE) %>%
                                         dplyr::select(all_of('plant_name_id',
                                                              'taxon_rank',
                                                              'taxon_status',
                                                              'family',
                                                              'taxon_name',
