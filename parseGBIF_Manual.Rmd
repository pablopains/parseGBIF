...
devtools::install_github("pablopains/parseGBIF", dempendeces = TRUE)
```
...
```{r example_download_gbif_data_from_doi, eval=TRUE, message=FALSE, warning=FALSE}
 ...
 download_gbif_data_from_doi(gbif_doi_url = 'https://doi.org/10.15468/dl.k7d3c9', folder = folder_download, keep_only_occurrence_file = TRUE)
 
 download_gbif_data_from_doi(gbif_doi_url = 'https://www.gbif.org/occurrence/download/0072170-260519110011954', folder = folder_download, keep_only_occurrence_file = TRUE)
...
### 6. Check coordinates
Checking coordinates, centroids, artificial points, also makes use of basic functions of CoordinateCleaner and other packages

```{r parse_coordinates, eval=TRUE, message=FALSE, warning=FALSE}

  occ_parse_coordinates <- parse_coordinates(occ = results$all_data,
...
summ <- parseGBIF_summary(parseGBIF_all_data = occ_parse_coordinates)
